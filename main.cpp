...
#define NUMPHASES 9 //Number of Phases
...
double phase_step;
double max_phase;
double max_disp; //Maximum Disparity
int phase_index;
...
for(int p=1; p<=number_directions ;p++){ //Loop runs for number of directions
  ...
  //std::cout<<"Theta Value : "<< value * M_PI <<std::endl; //Debug Code
...
max_disp = abs(*disparity_vector_it);
...
int vergence_control(double &disp_hor){
  ...
  double vergence_angle = (disp_hor/max_disp) * (90); //Using 90 Degree by default
  std::cout << "Disparity Received : " << disp_hor ;
  std::cout << "  Vergence Angle : "<< vergence_angle << std::endl;
  pos->setPositionMode();
  setpoints[5] = vergence_angle; //This is the proportional value for the disparity estimated to do vergence vontrol
...
int main(int argc, char *argv[])
{
  ...
  while(true){ //TODO wait till the input port receives data and run
    ...
    while(!q.empty()) //Do the computation till the queue is empty
    {
      ...
      //std::cout << "Number of events : " << num_events << std::endl; //Debug Code
      ...
      if(channel == 0 ){
        ...
        //std::cout << "Channel : " << channel << std::endl;//Debug Code
      ...
      /*for(disparity_est_theta_it=disparity_est_theta.begin(); disparity_est_theta_it!=disparity_est_theta.end();){

      std::cout << *disparity_est_theta_it << " ";
      disparity_est_theta_it++;
    ...
    //Clearing the Disparity Estimate Theta vector
    disparity_est_theta.clear();
  }
  else{
    continue;
...
void disparity_estimation(int &x,int &y){
  ...
  //Execute the vergence control call after 100 events are processed
  /*if(num_events > 50){

  vergence_control(Dx); //Calling vergence control routine
  num_events = 0; //Resetting the event count
